...
/*   Updated: 2020/02/10 19:49:01 by vdelsie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_scoreboard(t_envars *e, t_gslst *gs)
{
	...
	mlx_string_put(e->mlx, e->win, e->b_lpad, 10, 0x18FF0A, e->g->p1); // темно синий ///зеленый 
	mlx_string_put(e->mlx, e->win, e->b_lpad + p1_end, 10, 0x18FF0A, p1); /// темной синий //// зеленый
	mlx_string_put(e->mlx, e->win, e->b_lpad, 30, 0xFE1537, e->g->p2); /// синий ///красный
	mlx_string_put(e->mlx, e->win, e->b_lpad + p2_end, 30, 0xFE1537, p2); /// синий /// красный
	free(p1);
	free(p2);
}
...
void	render_piece_pos(t_envars *e, t_gslst *gs)
{
	...
	if (gs->prev)
	{
		...
		if (gs->prev->flags & P1_PLACED)
			mlx_string_put(e->mlx, e->win, x_start, 10, 0xFFA500, p_str); ////// серый
		else
			mlx_string_put(e->mlx, e->win, x_start, 10, 0x00A5FF, p_str); ///// светло-салатовый 
		free(x);
		free(y);
		free(p_str);
	}
	mlx_string_put(e->mlx, e->win, x_start, 30, 0xFFFFFF, "Next Piece"); /// розовый 
}
