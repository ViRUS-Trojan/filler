...
/*   Updated: 2020/02/10 19:35:31 by vdelsie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			choose_board_color(t_envars *e, int row, int col)
{
	...
	if (gs->board[row][col] == '.')
		color = 0xD3D3D3; /// светло-розовый
	else if (gs->board[row][col] == 'O')
		color = color_lint(0xFFC4C4, 0xFF0000, progress); ///// светло-желтый, блевотно зеленый 
	else if (gs->board[row][col] == 'o')
		color = 0xFFA500; ////  Фиолетовый 
	else if (gs->board[row][col] == 'X')
		color = color_lint(0xC4C4FF, 0x0000FF, progress);//// светло-розовый, желто-зеленый
	else if (gs->board[row][col] == 'x')
		color = 0x00A5FF;
	return (color);
}
...
int			choose_piece_color(t_envars *e, int row, int col)
{
	...
	color = 0;
	if (gs->piece[row][col] == '.')
		color = 0xD3D3D3; //// темно-зеленый
	else if (gs->piece[row][col] == '*')
	{
		if (gs->flags & P1_PLACED)
			color = 0xFFA500; //// Фиолетовый 
		else
			color = 0x00A5FF; //// Зеленый
	}
	return (color);
}
